...
def startFound(goalDistanceMap, currentPointList, startPoint):
    for currentPoint in currentPointList:
        if ((abs(currentPoint[0] - startPoint[0]) < 0.1 * goalDistanceMap.cell_size) and (
                abs(currentPoint[1] - startPoint[1]) < 0.1 * goalDistanceMap.cell_size)):
            return True
    return False
            ...
            if (probabilityMap.getProbabilityFromMap(i, j) > probabilityThreshold):
                ...
                for ii in range(-orthoCellDistance, orthoCellDistance + 1):
                    for jj in range(-orthoCellDistance, orthoCellDistance + 1):
                        if (cellX + ii < len(probabilityMap.mapa) and cellY + jj < len(
                                probabilityMap.mapa) and cellX + ii >= 0 and cellY + jj >= 0):
...
def chooseNeighbourDiagonal(goalDistanceMap, currentPoint, preferredDirection):
    chosenNeighbour = []
    currentCell = goalDistanceMap.get_cell(currentPoint[0], currentPoint[1])
    topleftNeighbour = goalDistanceMap.get_cell(currentPoint[0] - goalDistanceMap.cell_size,
                                                currentPoint[1] + goalDistanceMap.cell_size)
    toprightNeighbour = goalDistanceMap.get_cell(currentPoint[0] + goalDistanceMap.cell_size,
                                                 currentPoint[1] + goalDistanceMap.cell_size)
    bottomleftNeighbour = goalDistanceMap.get_cell(currentPoint[0] - goalDistanceMap.cell_size,
                                                   currentPoint[1] - goalDistanceMap.cell_size)
    bottomrightNeighbour = goalDistanceMap.get_cell(currentPoint[0] + goalDistanceMap.cell_size,
                                                    currentPoint[1] - goalDistanceMap.cell_size)
    if (preferredDirection == direction.slash):
        if (bottomleftNeighbour < currentCell):
            ...
            if (bottomrightNeighbour < currentCell):
                chosenNeighbour = [currentPoint[0] + goalDistanceMap.cell_size,
                                   currentPoint[1] - goalDistanceMap.cell_size]
...
def chooseNeighbour(goalDistanceMap, currentPoint, preferredDirection):
    ...
    chosenNeighbourDiagonal, preferredDirectionDiagonal = chooseNeighbourDiagonal(goalDistanceMap, currentPoint,
                                                                                  preferredDirection)
    chosenNeighbourOrthogonal, preferredDirectionOrthogonal = chooseNeighbourOrthogonal(goalDistanceMap, currentPoint,
                                                                                        preferredDirection)
    ...
    if (goalDistanceMap.get_cell(chosenNeighbourDiagonal[0], chosenNeighbourDiagonal[1]) <= goalDistanceMap.get_cell(
            chosenNeighbourOrthogonal[0], chosenNeighbourOrthogonal[1])):
        return chosenNeighbourDiagonal, preferredDirectionDiagonal
    else:
        return chosenNeighbourOrthogonal, preferredDirectionOrthogonal
...
def findPathPoints(goalDistanceMap, startPoint, goalPoint):
    ...
    while (not (abs(currentPoint[0] - goalPoint[0]) < 0.1 * goalDistanceMap.cell_size and abs(
            currentPoint[1] - goalPoint[1]) < 0.1 * goalDistanceMap.cell_size)):
    ...
    for i in range(0, len(WaveMap.mapa)):
        for j in range(0, len(WaveMap.mapa[0])):
            ...
            plt.scatter(point[0], point[1], c=color,
                        s=size[0] / (0.085 * WaveMap.world_latitude / WaveMap.cell_size) ** 2, marker='s', cmap='hsv',
                        alpha=pointValue)
